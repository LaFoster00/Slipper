...
namespace Slipper::Editor
{
    void Editor::Init()
    {
        ...
        Application::Get().AddViewportResizeCallback(
            [this](const NonOwningPtr<GPU::RenderingStage> Stage, const uint32_t Width, const uint32_t Height)
            { OnViewportResize(Stage, Width, Height); });
        m_graphicsEngine = &GPU::GraphicsEngine::Get();
        GPU::GraphicsEngine::GetDefaultCamera().AddComponent<EditorCameraComponent>();
    ...
    void Editor::OnViewportResize(NonOwningPtr<GPU::RenderingStage> Stage, uint32_t Width, uint32_t Height)
    {
    ...
    NonOwningPtr<Editor::ViewportData> Editor::FetchViewportImages(NonOwningPtr<GPU::RenderingStage> Stage)
    {
        ...
        const auto swap_chain = m_graphicsEngine->viewportRenderingStage.TryCast<GPU::Vulkan::VKRenderingStage>()
                                    ->TryGetSwapChain<GPU::Vulkan::OffscreenSwapChain>();
        ...
        {
            viewport_data->descriptors.push_back(ImGui_ImplVulkan_AddTexture(
                GPU::Vulkan::Sampler::GetLinearSampler(), viewport_image, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL));
        }

        return viewport_data;
    }

    void Editor::DrawViewport(GPU::RenderingStage &Stage)
    {
        static bool open = true;
        ...
        if (last_viewport_size.x != viewport_size.x || last_viewport_size.y != viewport_size.y)
        {
            Application::Get().OnViewportResize(
                &Stage, static_cast<uint32_t>(viewport_size.x), static_cast<uint32_t>(viewport_size.y));
        }
    ...
    void Editor::DrawGuizmo(GPU::RenderingStage &Stage, ImVec2 ViewportPos, ImVec2 ViewportSize)
    {
        if (SceneOutliner::IsEntitySelected())
        {
            ...
            const ImGuiWindowFlags window_flags = ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoDocking |
                ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoSavedSettings |
                ImGuiWindowFlags_NoFocusOnAppearing | ImGuiWindowFlags_NoNav | ImGuiWindowFlags_NoMove;
            ImGui::SetNextWindowPos(ViewportPos + ImVec2(10.0f, 10.0f), ImGuiCond_Always, {0.0f, 0.0f});
