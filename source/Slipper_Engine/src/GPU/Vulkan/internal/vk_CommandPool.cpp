...
{
    CommandPool::CommandPool(const vk::Queue Queue, const uint32_t QueueFamilyIndex, const int32_t BufferCount)
        : m_queue(Queue)
    {
        const vk::CommandPoolCreateInfo pool_info(vk::CommandPoolCreateFlagBits::eResetCommandBuffer, QueueFamilyIndex);
        VK_HPP_ASSERT(device.logicalDevice.createCommandPool(&pool_info, nullptr, &m_vkCommandPool),
    ...
    std::vector<vk::CommandBuffer> &CommandPool::CreateCommandBuffers(const int32_t BufferCount,
                                                                      int32_t *NewCommandBufferStartIndex)
    {
        if (BufferCount > 0)
        {
                                         ...
                                         const vk::CommandBufferUsageFlags Flags)
    {
        ...
        if (SingleUseBuffer &&
            std::ranges::find(m_singleUseVkCommandBuffers, CommandBuffer) != m_singleUseVkCommandBuffers.end())
        {
    ...
    vk::CommandBuffer CommandPool::CreateSingleUseCommandBuffer()
    {
        ...
        const vk::CommandBufferAllocateInfo alloc_info(m_vkCommandPool, vk::CommandBufferLevel::ePrimary, 1);
