...
namespace Slipper
{
    GPU::Vulkan::VKDevice *Gui::m_device = nullptr;
    ...
    Gui::Gui(std::string_view Name, NonOwningPtr<GPU::Vulkan::RenderPass> RenderPass, bool InstallCallbacks)
    ...
    void Gui::Init()
    {
        ...
        m_device = &GPU::Vulkan::VKDevice::Get();
        ...
        info.Instance = GPU::Vulkan::VulkanInstance::Get();
        ...
        info.MinImageCount = GPU::Vulkan::MAX_FRAMES_IN_FLIGHT;
        info.ImageCount = GPU::Vulkan::MAX_FRAMES_IN_FLIGHT;
        info.MSAASamples = static_cast<VkSampleCountFlagBits>(GPU::GraphicsSettings::MSAA_SAMPLES);
        ...
        {
            // Create font atlas
            GPU::Vulkan::SingleUseCommandBuffer command_buffer(*GPU::GraphicsEngine::Get().memoryCommandPool);
    ...
    void Gui::EndNewFrame(NonOwningPtr<GPU::RenderingStage> RenderingStage) const
    {
        ImGui::SetCurrentContext(m_context);
        ImGui::Render();
        const auto draw_data = ImGui::GetDrawData();

        RenderingStage->SubmitSingleDrawCommand(m_renderPass,
    ...
    void Gui::SetupDocksapce() const
    {
        ...
        ImGui::Begin(std::format("Dockspace Window {}", GetName()).c_str(), &window_open, window_flags);
        ImGui::PopStyleVar(3);
