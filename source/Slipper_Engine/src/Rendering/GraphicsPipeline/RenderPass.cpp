...
RenderPass::RenderPass(VkFormat AttachmentFormat, VkFormat DepthFormat): m_activeSwapChain(nullptr)
{
...
void RenderPass::CreateSwapChainFramebuffers(SwapChain *SwapChain)
{
    ...
    for (size_t i = 0; i < SwapChain->vkImageViews.size(); i++) {
        std::array attachments = {SwapChain->vkImageViews[i], SwapChain->depthBuffer->textureView};
        VkExtent2D extent = SwapChain->GetResolution();
        swapChainFramebuffers[SwapChain].emplace_back(
            std::make_unique<Framebuffer>(this, attachments.data(), static_cast<uint32_t>(attachments.size()), extent));
    }
}
